  ...
  useEffect(() => {
    if (success) {
      ...
      if (!product || product._id !== productId) {
        ...
        const newInstructions = product.description.instructions.map((ins) => ({
          targetPlants: {
            plants: ins.plants.map((plant) => ({
              name: plant.name,
              id: plant.id,
            })),
            object: ins.object,
        ...
        setidCategory(product.category.id);
        setSubcategory(product.subcategory.map((sub) => sub.id));
        setPrice(product.price);
        setCountInStock(product.countInStock);
        setUnit(product.unit);
        setCompany(product.company);
        setComponents(product.description.components);
        setUses(product.description.uses);
        setInstruction(newInstructions);
        setNote(product.description.note);
